import React from 'react';

export default function About() {
  return (
    <div>
      <h1>Welcome to</h1>
      <h1><b>Password Manager</b></h1>
      <br></br>
      <br />
      <h5>This is a website about checking how secure your passwords are.</h5>
      <h5>If a password are weak, you will be able to generate brand new secure one.</h5>
      <br />
      <br />
      <div className='homeAbout'>
        <h2>How to secure your online accounts?</h2>
        <h5>The best way to protect your accounts online is to create a strong password. You can check how strong your password is <a href="./ratePassword">here</a>. If you would like to read more information about protecting your account, click <a href="./Account_Security">here</a>.</h5>
        <br></br>
        <h2>How to make a strong password?</h2>
        <h5>If you want to create a strong password, try to avoid commonly used ones, such as: 12345678, qwerty, password, etc. You can also generate a new, strong password from <a href='/generatePassword'>here</a>. For more information about creating a strong password, click <a href='/Strong_Passwords'>here</a>.</h5>
      </div>
    </div>
  )
}
